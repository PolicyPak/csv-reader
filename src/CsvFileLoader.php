...
class CsvFileLoader implements LoaderInterface
{
     ...
     * @throws \Exception
     */
    public function getItemsWithSpecificHeaders($specificHeaders)
    {
        $this->openFile();

        $specificHeadersNumbers = array();
        ...
        if ($headers === false) {
            ...
            if ($headers === null) {
                ...
                for ($i = 0; $i < count($cols); $i++) {
                    if (in_array($cols[$i], $specificHeaders)) {
                        $headers[] = $cols[$i];
                        $specificHeadersNumbers[] = $i;
                    }
                }
            }
            $countHeaders = count($headers);
        ...
        while ($this->f && !feof($this->f)) {
            ...
            if ($headers === false) {
                ...
                $tempCols = array();
                foreach ($specificHeadersNumbers as $number) {
                    $tempCols[] = $cols[$number];
                }

                $item = array_combine($headers, $tempCols);
     ...
     * @throws \Exception
     */
    public function getItemsArrayWithSpecificHeaders($specificHeaders)
    {
        $result = [];

        foreach ($this->getItemsWithSpecificHeaders($specificHeaders) as $item) {
            $result[] = $item;
        }
